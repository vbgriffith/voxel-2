...
class Player {
    ...
    createMesh() {
        // Get 3D model from AssetManager
        const assetManager = this.engine.getSystem('assets');
        if (assetManager) {
            this.mesh = assetManager.getInstance('player');
            if (this.mesh) {
                this.mesh.position = this.position;
                this.weapon = this.mesh.getChildMeshes().find(m => m.name.includes('Weapon'));
            }
        }
        
        // Fallback to simple mesh if model not available
        if (!this.mesh) {
...
class Enemy {
    ...
    createMesh() {
        ...
        switch(this.type) {
            case 'grunt': modelName = 'enemyGrunt'; break;
            case 'fast': modelName = 'enemyFast'; break;
            case 'tank': modelName = 'enemyTank'; break;
            case 'ranged': modelName = 'enemyRanged'; break;
        }
        
        if (assetManager && modelName) {
            this.mesh = assetManager.getInstance(modelName);
            if (this.mesh) {
                this.mesh.position = this.position;
                return;
            }
    ...
    takeDamage(amount) {
        ...
        // Visual feedback - flash all child meshes
        const meshes = this.mesh.getChildMeshes ? this.mesh.getChildMeshes() : [this.mesh];
        meshes.forEach(childMesh => {
            if (childMesh.material) {
                const originalEmissive = childMesh.material.emissiveColor.clone();
                childMesh.material.emissiveColor = new BABYLON.Color3(1, 0.5, 0.5);
                
                setTimeout(() => {
                    if (childMesh && childMesh.material) {
                        childMesh.material.emissiveColor = originalEmissive;
                    }
                }, 100);
            }
        });
