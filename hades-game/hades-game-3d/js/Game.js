...
class Game {
    ...
    initSystems() {
        ...
        this.systems.assets.loadAllAssets().then(() => {
    ...
    createArena() {
        ...
        // Pillars for cover
        const assetManager = this.systems.assets;
        ...
        pillarPositions.forEach((pos, i) => {
            ...
            // Try to use enhanced model
            if (assetManager && assetManager.models.has('pillar')) {
                pillar = assetManager.getInstance('pillar');
            }
            
            // Fallback to simple pillar
            if (!pillar) {
                pillar = this.engine.createMesh('cylinder', `pillar_${i}`, {
                    height: 4,
                    diameter: 1.5
                });
    ...
    reset() {
        ...
        if (this.player) {
            ...
            if (this.player.mesh) {
                ...
                if (this.player.mesh.getChildMeshes) {
                    this.player.mesh.getChildMeshes().forEach(child => child.dispose());
                }
        ...
        this.enemies.forEach(e => {
            if (e.active) {
                if (e.mesh && e.mesh.getChildMeshes) {
                    e.mesh.getChildMeshes().forEach(child => child.dispose());
                }
